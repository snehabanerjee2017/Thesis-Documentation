...
\ac{PCA} is a statistical method that reduces the number of dimensions in a dataset while maintaining the majority of the original data, making complicated datasets easier to understand. It reduces a huge set of variables to smaller sets that retains the majority of the data from the original set. This is accomplished by generating new variables that are linear combinations of the original variables, known as main components. These principle components are uncorrelated and arranged so that the majority of the variation found in all of the original variables is retained in the first few. \cite*{mackiewicz1993principal}

\vspace{5mm}

Initially, the data is scaled to ensure that every variable makes an equal contribution to the study, a process known as standardization. The covariance matrix is then calculated to see how the dataset's variables differ from the mean in relation to one another. The covariance matrix's eigenvalues and eigenvectors are then computed. The new feature space's directions are determined by the eigenvectors, or primary components, and its magnitude is determined by the eigenvalues. The new features V are defined as in Eq. \ref*{eq:pca_feature}
\begin{equation}
  \label{eq:pca_feature}
  \mathit{V} = \mathit{A'X}
\end{equation}
where A is a matrix of orthonormal eigenvectors of the variance-covariance matrix S derived from the observations. The eigenvalues $\lambda_i$ of S correspond to the variances of the principal components. The feature vector, which is utilized to build the principal components, is created by sorting the eigenvectors in descending order based on their eigenvalues. Ultimately, this feature vector is used to convert the original data into the new feature space, producing a smaller dataset with fewer dimensions. One can utilize the coefficients of determination between the initial variables and the principal components to interpret the components and ascertain which initial variables account for the largest shares of variation in a given principal component.\cite*{jackson1973relationships}. For the variance-covariance matrix S, the coefficient of correlation is defined as in Eq. \ref*{eq:pca_coeff}
\begin{equation}
  \label{eq:pca_coeff}
  \mathit{r_{ij}} = \mathit{a_{ij}\sqrt{\frac{\lambda_i}{ S_{jj}}}}
\end{equation}
 Since the matrices employed in \ac{PCA}—variance-covariance and correlation—are merely estimators of the underlying population matrices, hypothesis testing is performed to confirm the findings. Since the size of the sample utilized may have a significant impact on the results, tests based on the asymptotic distributions of the roots and vectors of these matrices are used. \cite*{mackiewicz1993principal} 

\vspace{5mm}

\ac{PCA} reduces dimensionality, reduces noise, and enhances machine learning algorithm performance, among other advantages. Models are made simpler, which facilitates data exploration and visualization. \ac{PCA} has several uses, such as picture compression, which lowers the number of pixels while maintaining important features, genomics, which uses it to find patterns in gene expression data, and finance, which uses it to simplify financial data so that it may be more easily analyzed and predicted. \cite*{mackiewicz1993principal}

\vspace{5mm}

While \ac{PCA} is a useful method for reducing dimensionality, it has certain drawbacks. The assumption of linearity is one major flaw. In real-world data, it may not always be the case that the relationships between variables are linear, as \ac{PCA} assumes. This may reduce its ability to represent intricate, non-linear interactions.
