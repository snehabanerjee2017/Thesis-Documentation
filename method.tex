...
Once the feature representations were obtained, multiple clustering algorithms were utilized to group the objects in the datasets into clusters. 
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1] 
        ...
        \While{$True$}
            \State $\mathcal{E}_R, \; C, \; clusters, \; cluster\_members, \; num\_clusters \; classifier = get\_hier\_clusters(\mathcal{E}, \; \mathcal{E}_R, \; C, \; clusters, \; cluster\_members, count)$
            \State $count +=1$
...
\end{algorithm}
\begin{algorithm}[H]
    \caption{Pseudo-code for multi-level \ac{HDBSCAN} clustering}\label{alg:hier_cluster}
    ...
    \textbf{Output:} $\mathbf{return \;}\mathcal{E}_R, \; C, \; clusters, \; cluster\_members, \; num\_clusters, \; classifier$ 
    \begin{algorithmic}[1] 
        \If{$count \neq 0$}
            \State $classifier,\; predictions,\; num\_clusters = \ac{HDBSCAN}(\mathcal{E}_R)$
            ...
            \For{$\forall \; obj\_idx, \, object \in enumerate(\mathcal{E}_R)$}
                \If{$prediction[obj\_idx] < 0$}
                    ...
                    \For{$\forall \; members \in cluster\_members$}
                        \If{$object \in any(members)$}
                            \For{$\forall \; member \in members$}
                                \State $idx = where(member \in \{\mathcal{P}\})$
                                \State $C[idx] = cluster\_pointer$
                            \EndFor
                            \State $cluster\_pointer+=1$
                            \State $break$
                        \EndIf
                    \EndFor
                \ElsIf{$prediction[obj\_idx] \geq 0$}
                    \For{$\forall \; members \in cluster\_members$}
                        \If{$object \in any(members)$}
                            \For{$\forall \; member \in members$}
                                \State $idx = where(member \in \{\mathcal{P}\})$
                                \State $C[idx] = prediction[obj\_idx]$
                            \EndFor
                            \State $break$
                        \EndIf
                    \EndFor
            ...
            \State $clusters = list(unique(C))$
            \For{$\forall \; C_i \in clusters$}
                \If{$cluster > cluster\_pointer$}
                    \State $idx = where(cluster \in C)$
                    \State $C[idx] = cluster\_pointer$
                    \State $cluster\_pointer+=1$
                \EndIf
            \EndFor
        ...
        \EndIf
        \State $\mathbf{return \;}\mathcal{E}_R, \; C, \; clusters, \; cluster\_members, \; num\_clusters, \; classifier$ 
    \end{algorithmic}
\end{algorithm}
